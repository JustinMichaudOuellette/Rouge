...
`release.p12`, in the repository root, with a random password stored next to
it in `release.p12.pass`. Both are gitignored (`*.p12`, `*.p12.pass`); a
keystore left over from the old `build/keys/release.p12` location is moved to
the root on the next run instead of being replaced, because `gradlew clean`
(and Android Studio's Clean Project) deletes `build/`:

- **Back the two files up.** If you lose them, you can never ship an update
  over an installed or published copy (Android requires the same key). They
  are the one thing in this repository that cannot be regenerated.
- The same run writes `release.p12.sha256` — the certificate's SHA-256
  fingerprint, one line, uppercase hex, colon-separated, no `SHA256:` prefix,
  which is exactly what the Play Console's certificate fingerprint field
  accepts, so it can be copied straight out of the file:

  ```bash
  cat release.p12.sha256
  # C4:20:94:80:03:1A:3E:C4:0D:86:B4:5E:3D:13:20:95:BD:2A:70:8A:91:C8:AF:30:02:96:CC:6C:43:07:E3:20
  # ^ this repository's current key; the file is what to read, not this line
  ```

  It is rewritten on every run (and by `--recert`) because the fingerprint
  *is* the signing identity — a stale file would name the wrong certificate.
  Unlike the keystore it is public information and is meant to be committed.
  `release.py --recert --no-build --no-install` refreshes nothing but the
  certificate and that file.
...
- For Play Store / long-lived apps, generate one key once and keep it; pass
  it explicitly with `--ks` / `--ks-pass` (the fingerprint file follows the
  keystore: `--ks your.p12` writes `your.p12.sha256`).
